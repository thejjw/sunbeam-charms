...
test_directories = ["set1"]
built_charms = glob.glob("*.charm")
context = {
    charm.strip(".charm").replace("-", "_"): True for charm in built_charms
}

for test_dir in test_directories:
    bundle_dir = f"tests/{test_dir}/bundles"
    template_loader = Environment(loader=FileSystemLoader(bundle_dir))
    bundle_template = template_loader.get_template("smoke.yaml.j2")
    with open(
        f"{bundle_dir}/smoke.yaml", mode="w", encoding="utf-8"
    ) as content:
