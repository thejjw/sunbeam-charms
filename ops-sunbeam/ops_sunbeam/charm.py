...
def canonical_snap_channel(channel: str) -> str:
    ...
    A risk/branch shorthand ("edge/hotfix") means "latest/edge/hotfix".
    """
...
def resolve_snap_channel(installed: str, configured: str) -> str:
    """Resolve a configured channel against the installed snap's track.

    When the configured channel omits the track (bare risk like "candidate"
    or risk/branch like "edge/hotfix"), prepend the installed channel's
    track so that a bare "candidate" against an "2026.1/stable" install
    resolves to "2026.1/candidate" instead of "latest/candidate".

    This is a charm-level convention: snapd natively resolves bare
    "candidate" to "latest/candidate". We resolve against the installed
    track because operators on a versioned track (2026.1) expect risk
    changes to stay on that track without restating it.
    """
    if not installed or not configured:
        return configured
    first = configured.split("/")[0]
    if first in _SNAP_RISKS:
        return f"{snap_track(installed)}/{configured}"
    return configured
...
def snap_channel_drift_message(installed: str, configured: str) -> str:
    ...
    if not installed or not configured:
        return ""
    resolved = resolve_snap_channel(installed, configured)
    if canonical_snap_channel(installed) == canonical_snap_channel(resolved):
        return ""
    ...
    return f"installed snap: {installed}, configured: {resolved}"
...
class OSBaseOperatorCharm(
    ...
    def __init__(self, framework: ops.framework.Framework) -> None:
        ...
        # If the charm supports peer relation, paused state will be stored in
        # peer relation data bag, and synced across all units, so that the
        # paused state can be preserved even when the pod is replaced.
        # This is not an issue for machine charms as the machine is not
        # replaced during upgrades.
        self._state.set_default(paused=False)
    ...
    def is_service_paused(self) -> bool:
        ...
        if self.supports_peer_relation:
            ...
            services_paused = bool(self._state.paused)
    ...
    def _on_resume_action(self, event: ops.ActionEvent) -> None:
        ...
        if not self._configure_charm_completed:
            event.fail("Failed to resume service, check logs for details")
            return
...
class OSBaseOperatorAPICharm(OSBaseOperatorCharmK8S):
    ...
    def _on_pause_action(self, event: ops.ActionEvent) -> None:
        ...
        try:
            ...
            event.fail(f"Failed to set peer paused state: {e}")
            return

        for ph in self.pebble_handlers:
            ph.stop_healthcheck("up")

        super()._on_pause_action(event)
    ...
    def _on_resume_action(self, event: ops.ActionEvent) -> None:
        ...
        try:
            ...
            event.fail(f"Failed to clear peer paused state: {e}")
            return

        for ph in self.pebble_handlers:
            ph.start_healthcheck("up")

        super()._on_resume_action(event)
...
class OSBaseOperatorCharmSnap(OSBaseOperatorCharm):
    ...
    def _ensure_snap_confinement_change(
        ...
        if snap_svc.channel:
            resolved = resolve_snap_channel(
                snap_svc.channel, self.snap_channel
            )
            if snap_track(snap_svc.channel) != snap_track(resolved):
                ensure_channel = snap_svc.channel
            else:
                ensure_channel = resolved
    ...
    def _on_refresh_snap_action(self, event: ops.ActionEvent) -> None:
        ...
        try:
            snap_svc = self.get_snap()
            if snap_svc.present and snap_svc.channel:
                channel = resolve_snap_channel(snap_svc.channel, channel)
            snap_svc.unhold()
            snap_svc.ensure(
                self.snap_module.SnapState.Latest,
                channel=channel,
