...
class TestUserIdentityResourceRequiresHandler(test_utils.CharmTestCase):
    ...
    def test_process_create_user_response_same_user_no_delete(self) -> None:
        """Re-creating the same user (pod restart) must NOT queue it for deletion."""
        # Pre-existing config credentials with the SAME username
        self._store_credentials("test-user", "secret:old-pw")
        self._store_config_secret("secret:cfg1")
        ...
        # test-user must NOT appear in delete list
        old_users = json.loads(self._leader_data.get("old_users", "[]"))
        self.assertNotIn("test-user", old_users)

        # config secret should still be updated with new password
        mock_cfg_secret.set_content.assert_called_once_with(
            {"username": "test-user", "password": "new-pass"}
        )
    ...
    def test_on_provider_goneaway_clears_state_and_calls_cleanup(self) -> None:
        """Goneaway clears references, queues the user, and runs cleanup."""
        self._store_credentials(
            username="old-user", password_secret="secret:pw"
        )
        self._store_config_secret("secret:cfg1")
        self.handler._on_goneaway_cleanup = MagicMock()
        config_secret = MagicMock()
        self.mock_charm.model.get_secret.return_value = config_secret
        ...
        self.assertEqual(self._leader_data[self.handler.config_label], "")
        self.mock_charm.model.get_secret.assert_called_once_with(
            id="secret:cfg1"
        )
        config_secret.remove_all_revisions.assert_called_once()
        self.handler._on_goneaway_cleanup.assert_called_once_with(event)
        old_users = json.loads(self._leader_data["old_users"])
        self.assertIn("old-user", old_users)
        self.mock_callback.assert_called_once_with(event)

    def test_on_provider_goneaway_supports_legacy_secret_reference(
        self,
    ) -> None:
        """Legacy bare secret ids still resolve the username for cleanup."""
        self._leader_data[self.handler.credentials_secret_label] = (
            "secret:old123"
        )
        self.handler._on_goneaway_cleanup = MagicMock()
        ...
        legacy_secret.get_content.return_value = {"username": "legacy-user"}
        self.mock_charm.model.get_secret.return_value = legacy_secret
        ...
        old_users = json.loads(self._leader_data["old_users"])
        self.assertIn("legacy-user", old_users)
        self.assertEqual(
            self._leader_data[self.handler.credentials_secret_label], ""
        )
        self.handler._on_goneaway_cleanup.assert_called_once_with(event)
    ...
    def test_on_provider_goneaway_ignores_malformed_credentials(self) -> None:
        """Malformed leader data does not block cleanup."""
        self._leader_data[self.handler.credentials_secret_label] = "not-json"
        self._store_config_secret("secret:cfg1")
        self.handler._on_goneaway_cleanup = MagicMock()
        config_secret = MagicMock()
        self.mock_charm.model.get_secret.return_value = config_secret
        ...
        self.assertNotIn("old_users", self._leader_data)
        self.assertEqual(
            self._leader_data[self.handler.credentials_secret_label], ""
        )
        self.assertEqual(self._leader_data[self.handler.config_label], "")
    ...
    def test_on_provider_goneaway_tolerates_missing_config_secret(
        self,
    ) -> None:
        """Missing config secrets are ignored while state is still cleared."""
        self._store_credentials(
            username="old-user", password_secret="secret:pw"
        )
        self._store_config_secret("secret:cfg1")
    ...
    def test_on_provider_goneaway_non_leader_is_noop(self) -> None:
        """Non-leader units do no cleanup work."""
        self.mock_charm.model.unit.is_leader.return_value = False
        self._store_credentials(
            username="old-user", password_secret="secret:pw"
        )
        ...
        self.assertEqual(
            ...
            {"username": "old-user", "password": "secret:pw"},
        )
        self.assertEqual(
            self._leader_data[self.handler.config_label], "secret:cfg1"
        )
        self.assertNotIn("old_users", self._leader_data)
        self.mock_charm.model.get_secret.assert_not_called()
        self.handler._on_goneaway_cleanup.assert_not_called()
        self.mock_callback.assert_called_once_with(event)
