...
then
	...
	pushd ops-sunbeam
	coverage erase
        PYTHON="coverage run --parallel-mode --omit .tox/*,tests/*" stestr run --slowest || exit 1
	coverage combine
        popd
	...
	charms=($(ls charms))
		...
		push_common_files $charm || exit 1
                pushd charms/$charm
		coverage erase
                PYTHONPATH=./src:./lib:../../ops-sunbeam PYTHON="coverage run --parallel-mode --omit .tox/*,tests/*,src/templates/*" stestr run --slowest || exit 1
		coverage combine
                popd
        done
