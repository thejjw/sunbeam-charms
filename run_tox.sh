...
then
	...
	for charm in ${charms[@]}; do
		pushd charms/$charm
		PYTHONPATH=./src:./lib:../../ops-sunbeam stestr run --slowest || exit 1
		popd
	done
elif [[ $1 == "cover" ]];
then
	...
	pushd ops-sunbeam
	coverage erase
        PYTHON="coverage run" stestr run --slowest || exit 1
	coverage combine
        popd
	...
	charms=($(ls charms))
        for charm in ${charms[@]}; do
                pushd charms/$charm
		coverage erase
                PYTHONPATH=./src:./lib:../../ops-sunbeam PYTHON="coverage run" stestr run --slowest || exit 1
		coverage combine
                popd
        done
