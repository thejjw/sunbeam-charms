...
def _create_security_groups(neutron_client, keystone_client):
    ...
    # Default egress-all (IPv4 + IPv6) rules are created automatically by
    # Neutron when the security group is created.
...
def _configure_octavia(juju_k8s, network_id, flavor_id, lb_sg_id, hm_sg_id):
    ...
    # Two-phase wait so we reliably observe that config-changed has been
    # processed regardless of how fast the hook runs:
    #
    # Phase 1: wait up to 60 s for the config-changed hook to start
    #   (agent goes non-idle).  If the hook fires and finishes before our
    #   first poll we catch TimeoutError and proceed - octavia is already
    #   settled at that point and phase 2 returns immediately.
    # Phase 2: wait for workload=active.
    logging.info("Waiting for octavia config-changed hook to dispatch")
    try:
        juju_k8s.wait(
        ...
        logging.info(
            "octavia agent did not go non-idle within 60 s; "
