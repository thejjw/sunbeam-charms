...
class KeystoneAPIAuditTest(OpenStackAPIAuditTest):
    ...
    def _trigger_audit_event(self):
        ...
        user = cls.keystone_client.users.find(name=username)

        rand_num = random.randint(0, 32768)
        email = f"test{rand_num}.example.com"
        cls.keystone_client.users.update(user.id, email=email)
...
class BarbicanAPIAuditTest(AuditMiddlewareTest):
    ...
    def _trigger_audit_event(self):
        barbican_endpoint = cls.keystone_client.service_catalog.url_for(
            service_type='key-manager', interface='publicURL')
        barbican = barbican_client.Client(session=cls.keystone_session,
...
class WatcherAPIAuditTest(AuditMiddlewareTest):
    application_name = "watcher"

    def _trigger_audit_event(self):
        client = openstack_utils.get_watcher_session_client(
            self.keystone_session)
        client.action.list()
