    ...
    def test_100_snap_installed(self):
        ...
        for unit in units:
            with self.subTest(unit=unit):
                config = model.get_application_config(self.application_name)
                snap_name = config.get("snap-name", {}).get("value", "epa-orchestrator")
                
                logging.info("Checking if snap %s is installed on %s", snap_name, unit)
                self.assertTrue(
                    self._check_snap_installed(unit, snap_name),
                    f"Snap {snap_name} is not installed on {unit}"
                )
