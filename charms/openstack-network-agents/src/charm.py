...
class OpenstackNetworkAgentsOperatorCharm(
    ...
    def snap_name(self) -> str:
        ...
        return str(
            self.model.config.get("snap-name") or "openstack-network-agents"
        )

    @property
    def snap_channel(self) -> str:
        """Channel to track in the Snap Store."""
        return str(self.model.config.get("snap-channel") or "latest/edge")
    ...
    def _connect_ovn_chassis(self) -> None:
        ...
        openstack_network_agents = cache.get("openstack-network-agents")

        try:
            microovn = cache.get("microovn")
            if not microovn.present:
                logger.info("microovn not installed, skipping connection")
                return
        except snap.SnapNotFoundError:
            logger.info(
                "microovn snap not found in snap cache, skipping connection"
            )
            return
    ...
    def _validated_network_config(self) -> tuple[str, str, str, bool]:
        ...
        """
        cfg = self.model.config.get
        iface = cfg("external-interface")
        bridge = cfg("bridge-name")
        physnet = cfg("physnet-name")
        enable_gw = cfg("enable-chassis-as-gw")
