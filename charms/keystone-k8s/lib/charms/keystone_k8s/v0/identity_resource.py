...
class IdentityResourceRequires(Object):
    ...
    def _emit_provider_readiness(self, event: RelationEvent):
        """Handle IdentityResource readiness changes."""
        if self.resassert_provider_readiness():
            logger.debug("Emitting provider_ready event")
            self.on.provider_ready.emit(event.relation)

    def resassert_provider_readiness(self):
        """Reassert provider readiness.

        Return True, if the reassert changed the internal state to ready.
        """
    ...
    def provider_ready(self) -> bool:
        ...
        """
        try:
            return self.get_remote_app_data("status") == STATUS_READY
        except ops.ModelError as e:
            logger.debug("Failure to check provider readiness: %s", str(e))
            return False
