...
class TestGetIdpFileNameFromIssuerUrl:
    ...
    def test_preserves_host_with_port_over_http(self):
        """Regression for the original bug: http URL with port and an
        s-prefixed host must keep all characters and percent-encode the port
        separator."""
        from charm import (
            _BaseIDPHandler,
        )
    ...
    def test_preserves_host_with_port_over_https(self):
        """https URL with a port encodes the colon and keeps the host intact."""
        from charm import (
            _BaseIDPHandler,
        )
    ...
    def test_falls_back_when_url_has_no_netloc(self):
        """Inputs without a parseable scheme://netloc fall through to the
        raw-string branch."""
        from charm import (
            _BaseIDPHandler,
        )
...
class TestOIDCScope:
    ...
    def test_apache_template_renders_context_scope(self):
        """OIDCScope comes from fid.oidc_scope."""
...
class TestListCaCertsAction:
    ...
    def test_received_relation_bundle_decodes_json_quoted_ca(
        self, ctx, complete_state
    ):
        """A JSON-encoded CA string from relation data is normalized."""
        state_mid = _bootstrap(ctx, complete_state)
        ca_pem, _ = _get_self_signed()
        receive_ca_rel = testing.Relation(
            endpoint="receive-ca-cert",
            remote_app_name="certificate-authority",
            remote_units_data={
                0: {"ca": json.dumps(ca_pem), "chain": json.dumps([])}
            },
        )
        state_with_rel = dataclasses.replace(
            state_mid,
