...
class IdentityEndpointsProvidesHandler(sunbeam_rhandlers.RelationHandler):
    ...
    def ready(self) -> bool:
        """Check if handler is ready."""
        # Consider it to be ready if there are Keystone endpoints registered.
        try:
            return bool(self.charm.keystone_manager.ksclient.list_endpoint())
        except Exception as ex:
            logger.debug(
                "Keystone endpoint provider not ready yet, error: %r", ex
            )
            return False
...
"""
    ...
    def get_relation_handlers(
        ...
        if self.can_add_handler(self.IDENDP_RELATION_NAME, handlers):
            # BasePeerHandler needs to be initialized first.
