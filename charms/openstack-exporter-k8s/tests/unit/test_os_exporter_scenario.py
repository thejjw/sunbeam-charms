...
"""Scenario (ops.testing state-transition) tests for openstack-exporter-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → clouds.yaml is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out,
            ctx,
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("openstack-exporter")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("openstack-exporter")
        assert "openstack-exporter" in out_container.layers
        layer = out_container.layers["openstack-exporter"]
        assert "openstack-exporter" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("openstack-exporter") == (
    ...
    def test_pebble_ready_without_relations_blocked(self, ctx):
        ...
        container = k8s_container("openstack-exporter")
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = k8s_container("openstack-exporter", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
