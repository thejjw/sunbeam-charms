...
class TestNovaMetadataEndpoint:
    ...
    def test_nova_service_relation_includes_metadata_endpoint(self, ctx):
        ...
        secret_id = relation.local_app_data["metadata-proxy-shared-secret-id"]
        assert secret_id
        secret = state_out.get_secret(id=secret_id)
        assert secret.latest_content["metadata-proxy-shared-secret"]
