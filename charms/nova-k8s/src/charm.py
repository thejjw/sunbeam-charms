    ...
    def _publish_app_data(self, scheme, port, relation):
        ...
        try:
            IngressRequirerAppData(
                ...
                name=(
                    f"{self.model.app.name}" f"{NOVA_METADATA_INGRESS_SUFFIX}"
                ),
                scheme=scheme,
                port=port,
...
class NovaConductorPebbleHandler(sunbeam_chandlers.ServicePebbleHandler):
    ...
    def get_healthcheck_layer(self) -> dict:
        """Nova Conductor health check layer.

        Keep this separate from the base Pebble layer so the check is only
        installed after the charm has completed service configuration.
        """
        return {
            "checks": {
