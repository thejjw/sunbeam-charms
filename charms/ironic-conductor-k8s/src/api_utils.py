...
def create_keystone_session(
    ...
    """Create a Keystone session from the identity-credentials relation."""
    plugin_args = {
...
class OSClients(object):
    """Class containing OpenStack clients and methods needed by the charm."""
    ...
    def glance_stores(self):
        """Returns OpenStack Glance's configured stores."""
        return [stor["id"] for stor in self._stores_info]

    def get_default_glance_store(self):
        """Returns OpenStack Glance's default store."""
    ...
    def get_object_account_properties(self):
        """Returns the OpenStack Swift account properties."""
    ...
    def set_object_account_property(self, prop, value):
        """Sets the given property and value in the OpenStack Swift's account."""
    ...
    def delete_object_account_property(self, prop):
        """Deletes the given OpenStack Swift account property."""
        prop = prop.lower()
        meta_key = "x-account-meta-%s" % prop
        headers = {meta_key: ""}
        self._obj_cli.post_account(headers)
    ...
    def has_swift(self):
        """Returns whether OpenStack has a Swift endpoint registered."""
        return self._has_service_type("object-store")

    def has_glance(self):
        """Returns whether OpenStack has a Glance endpoint registered."""
