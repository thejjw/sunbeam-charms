...
class TestS3Backend:
    """Glance backed by S3: active without Swift/temp-url and serves locally."""

    def test_active_with_s3_no_temp_url(
        self, ctx, complete_state_s3, glance_s3_backend
    ):
        """Glance exposes an S3 store, no temp-url secret/ceph-rgw → ActiveStatus."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state_s3)
        assert state_out.unit_status == testing.ActiveStatus("")

    def test_image_download_source_local(
        self, ctx, complete_state_s3, glance_s3_backend
    ):
        """With an S3 backend, the agent downloads images locally (not swift)."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state_s3)
        assert_config_file_contains(
            state_out,
            ctx,
    ...
    def test_ipxe_use_swift_disabled(
        self, ctx, complete_state_s3, glance_s3_backend
    ):
        """With an S3 backend and iPXE, deploy images are not pulled from swift."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state_s3)
        assert_config_file_contains(
            state_out,
            ctx,
    ...
    def test_blocks_when_glance_not_s3(self, ctx, complete_state_s3):
        """Glance has no S3 store and no swift/temp-url → blocked."""
        with mock.patch("api_utils.create_keystone_session"), mock.patch(
            "api_utils.OSClients"
        ) as mock_os:
            mock_os.return_value.has_glance.return_value = True
            mock_os.return_value.glance_stores = ["file"]
            state_out = ctx.run(ctx.on.config_changed(), complete_state_s3)
