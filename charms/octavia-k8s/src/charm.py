...
class OctaviaOperatorCharm(sunbeam_charm.OSBaseOperatorAPICharm):
    ...
    def _get_second_interface_ip(self) -> str | None:
        """Get IP address of the second network interface.

        Determines the primary interface via default route and returns
        the IP of the first non-primary, non-loopback interface.

        :returns: IP address of second interface or None if not found
        :rtype: str | None
        """
        primary_iface = None
        try:
            ...
            if (
                "default" in gateways
                and netifaces.AF_INET in gateways["default"]
            ):
                primary_iface = gateways["default"][netifaces.AF_INET][1]
                logger.info(
                    f"Primary interface from default route: {primary_iface}"
                )
        ...
        exclude_ifaces = ["lo"]
        if primary_iface:
            exclude_ifaces.append(primary_iface)

        secondary_interfaces = [
            iface
            for iface in netifaces.interfaces()
            if iface not in exclude_ifaces
        ]

        if not secondary_interfaces:
            logger.info("No second network interface found")
            return None

        second_iface = secondary_interfaces[0]
        logger.info(f"Found second interface: {second_iface}")

        try:
            addrs = netifaces.ifaddresses(second_iface)
            if netifaces.AF_INET in addrs:
                ip_addr = addrs[netifaces.AF_INET][0]["addr"]
                logger.info(
                    f"Second interface {second_iface} has IP: {ip_addr}"
                )
                return ip_addr
        except (ValueError, KeyError, IndexError) as e:
            logger.error(f"Error getting IP for interface {second_iface}: {e}")
            return None
    ...
    def _set_lbmgmt_ip(self) -> None:
        ...
        lbmgmt_ip = self._get_second_interface_ip()
        if lbmgmt_ip:
