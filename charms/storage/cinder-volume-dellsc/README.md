...
juju config cinder-volume-dellsc \
  san-ip=10.0.0.50 \
  san-login=secret:dellsc-config \
  san-password=secret:dellsc-config \
  dell-sc-ssn=64702 \
  protocol=fc \
  volume-backend-name=dellsc01
```

You may also provide dedicated secrets that contain a single key/value pair
(`san-login`, `san-password`, `secondary-san-login`, `secondary-san-password`).
...
juju config cinder-volume-dellsc \
  ...
  secondary-san-login=secret:dellsc-config \
  secondary-san-password=secret:dellsc-config \
...
| `san-login`                        | secret  | –       | Secret with `san-login` key.                              |
| `san-password`                     | secret  | –       | Secret with `san-password` key.                           |
| `dell-sc-ssn`                       | int     | –       | Storage Center system serial number.                      |
| `protocol`                         | string  | `fc`    | `fc` or `iscsi` driver variant.                           |
| `enable-unsupported-driver`        | boolean | `true`  | Must be true for the Dell SC driver.                      |
...
| `secondary-san-login`              | secret  | –       | Secret with `secondary-san-login` key.                    |
| `secondary-san-password`           | secret  | –       | Secret with `secondary-san-password` key.                 |
