...
class TestCinderDellSCOperatorCharm(test_utils.CharmTestCase):
    ...
    def test_backend_config_maps_credentials(self):
        """Ensure credential secrets are expanded into backend config."""
        self.harness.begin_with_initial_hooks()
        primary_secret = self.harness.add_user_secret(
            {"username": "admin", "password": "secret"}
        )
        secondary_secret = self.harness.add_user_secret(
            {"username": "admin2", "password": "secret2"}
        )
        add_complete_cinder_volume_relation(self.harness)
        self.harness.grant_secret(primary_secret, self.harness.charm.app)
        self.harness.grant_secret(secondary_secret, self.harness.charm.app)
        self.harness.update_config(
            {
                "san-ip": "10.20.20.3",
                "dell-sc-ssn": 64702,
                "protocol": "fc",
                "san-credentials-secret": primary_secret,
                "secondary-san-ip": "10.20.20.4",
                "secondary-san-credentials-secret": secondary_secret,
            }
        )
        ...
        assert "san-credentials-secret" not in backend_config
        assert "secondary-san-credentials-secret" not in backend_config

    def test_backend_config_maps_single_secret(self):
        """Ensure combined DellSC secret is expanded into backend config."""
        self.harness.begin_with_initial_hooks()
        ...
        self.harness.update_config(
            {
                ...
                "dell-sc-ssn": 64702,
                "protocol": "fc",
                "dellsc-config-secret": combined_secret,
                "secondary-san-ip": "10.20.20.4",
            }
        )
        ...
        assert backend_config["secondary-san-login"] == "admin2"
        assert backend_config["secondary-san-password"] == "secret2"
        assert "dellsc-config-secret" not in backend_config
