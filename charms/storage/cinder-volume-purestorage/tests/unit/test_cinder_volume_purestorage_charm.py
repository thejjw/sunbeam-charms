...
class _CinderVolumePureStorageOperatorCharm(charm.CinderVolumePureStorageOperatorCharm):
...
class TestCinderPureStorageOperatorCharm(test_utils.CharmTestCase):
    ...
    def setUp(self):
        ...
        snap_patch = patch.object(
            _CinderVolumePureStorageOperatorCharm,
            "_import_snap",
    ...
    def test_all_relations(self):
        ...
        secret = self.harness.add_user_secret({"token": "my"})
        add_complete_cinder_volume_relation(self.harness)
        self.harness.grant_secret(secret, self.harness.charm.app)
        self.harness.update_config({"san-ip": "10.20.20.3", "pure-api-token": secret})
