...
"""Unit tests for Cinder HPE 3Par operator charm class."""
...
class _CinderVolumeHpethreeparOperatorCharm(
...
class TestCinderHpethreeparOperatorCharm(test_utils.CharmTestCase):
    """Test cases for CinderHpethreeparOperatorCharm class."""
    ...
    def setUp(self):
        ...
        snap_patch = patch.object(
            _CinderVolumeHpethreeparOperatorCharm,
            "_import_snap",
    ...
    def test_all_relations(self):
        ...
        usr = self.harness.add_user_secret({"san-login": "username"})
        secret = self.harness.add_user_secret({"san-password": "pwd"})
        add_complete_cinder_volume_relation(self.harness)
        self.harness.grant_secret(secret, self.harness.charm.app)
        self.harness.update_config(
                {"san-ip": "10.20.20.3", "san-login": usr, "san-password": secret}
        )
