...
"""Unit tests for Cinder synology operator charm."""
...
class _CinderVolumeSynologyOperatorCharm(charm.CinderVolumeSynologyOperatorCharm):
...
class TestCinderSynologyOperatorCharm(test_utils.CharmTestCase):
    """Test cases for CinderVolumeSynologyOperatorCharm class."""
    ...
    def setUp(self):
        ...
        snap_patch = patch.object(
            _CinderVolumeSynologyOperatorCharm,
            "_import_snap",
    ...
    def test_all_relations(self):
        ...
        secret_password = self.harness.add_user_secret({"synology-password": "test-password"})
        secret_otp = self.harness.add_user_secret({"synology-one-time-pass": "test-otp"})
        add_complete_cinder_volume_relation(self.harness)
        self.harness.grant_secret(secret_password, self.harness.charm.app)
        self.harness.grant_secret(secret_otp, self.harness.charm.app)
        # Update config with required fields and the secret reference
        self.harness.update_config(
            {"san-ip": "10.20.20.3", "synology-password": secret_password, "synology-one-time-pass": secret_otp}
        )
