...
"""Shared fixtures for cinder-volume-dellpowerstore unit tests (ops.testing)."""
...
@pytest.fixture()
def ctx():
    """Create a testing.Context for CinderVolumeDellPowerStoreOperatorCharm."""
...
def complete_state():
    ...
    san_ip_secret = testing.Secret(
        tracked_content={"san-ip": "10.20.30.40"},
        owner="app",
    )
    san_login_secret = testing.Secret(
        tracked_content={"san-login": "admin"},
        owner="app",
    )
    san_password_secret = testing.Secret(
        tracked_content={"san-password": "dangerous"},
        owner="app",
    )
    return testing.State(
        leader=True,
        config={
            "san-ip": san_ip_secret.id,
            "san-login": san_login_secret.id,
            "san-password": san_password_secret.id,
            "protocol": "fc",
        },
        relations=[cinder_volume_relation()],
        secrets=[san_ip_secret, san_login_secret, san_password_secret],
    )
