...
"""Scenario (ops.testing state-transition) tests for aodh-k8s."""
...
def _all_containers_disconnected():
    """All containers in can_connect=False state."""
    containers = [
        k8s_api_container(
            "aodh-api",
            can_connect=False,
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → aodh.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "aodh-api", "/etc/aodh/aodh.conf"
        )
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("aodh-api")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

    def test_pebble_ready_without_relations_blocked(self, ctx, all_containers):
        ...
        for c in all_containers:
            if c.name == "aodh-api":
                container_obj = c
                break
        state_in = testing.State(leader=True, containers=all_containers)
        state_out = ctx.run(ctx.on.pebble_ready(container_obj), state_in)
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        containers = _all_containers_disconnected()
