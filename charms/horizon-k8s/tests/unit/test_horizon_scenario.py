...
"""Scenario (ops.testing state-transition) tests for horizon-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → local_settings.py is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out,
            ctx,
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("horizon")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("horizon")
        assert "horizon" in out_container.layers
        layer = out_container.layers["horizon"]
        assert "wsgi-horizon" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("wsgi-horizon") == (
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = k8s_api_container("horizon", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
...
class TestActions:
    """Action tests for horizon-k8s."""

    def test_get_dashboard_url_action(self, ctx, complete_state):
        """Test the get-dashboard-url action returns the correct URL."""
        ctx.run(ctx.on.action("get-dashboard-url"), complete_state)
        assert ctx.action_results == {"url": "http://public-url:80/"}
