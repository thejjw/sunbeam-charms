...
"""Scenario (ops.testing state-transition) tests for nova-ironic-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → nova.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "nova-ironic", "/etc/nova/nova.conf"
        )

    def test_rootwrap_config_file_written(self, ctx, complete_state):
        """All relations present → rootwrap.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("nova-ironic")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("nova-ironic")
        assert "nova-ironic" in out_container.layers
        layer = out_container.layers["nova-ironic"]
        assert "nova-ironic" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("nova-ironic") == (
    ...
    def test_pebble_ready_without_relations_blocked(self, ctx):
        ...
        container = k8s_container("nova-ironic")
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = k8s_container("nova-ironic", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
