...
class SunbeamClusterdCharm(sunbeam_charm.OSBaseOperatorCharm):
    ...
    def __init__(self, framework: ops.Framework) -> None:
        ...
        self._state.set_default(channel="config")
    ...
    def ensure_snap_present(self):
        ...
        try:
            ...
            if not openstack.present or snap_channel != openstack.channel:
                ...
                self.set_workload_version(openstack)
    ...
    def set_workload_version(self, snap: snap.Snap):
        """Set workload version."""
        if not snap.present:
            return
        version = snap.channel + f"(rev {snap.revision})"
    ...
    def add_node_to_cluster(self, event: ClusterdNewNodeEvent) -> None:
        ...
        if event.unit is None:
            ...
            return
        token = self._clusterd.generate_token(self.unit.name.replace("/", "-"))
    ...
    def remove_node_from_cluster(self, event: ClusterdRemoveNodeEvent) -> None:
        ...
        if event.unit is None:
            ...
            return
        self._clusterd.remove_node(event.unit.name.replace("/", "-"))
