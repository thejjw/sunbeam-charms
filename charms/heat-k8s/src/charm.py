...
class HeatAPIPebbleHandler(sunbeam_chandlers.WSGIPebbleHandler):
    ...
    def init_service(self, context: sunbeam_core.OPSCharmContexts) -> None:
        ...
        try:
            process = container.exec(["a2dissite", "heat-api"], timeout=5 * 60)
            ...
            logger.exception("Failed to disable heat-api site in apache")
    ...
    def init_service(self, context: sunbeam_core.OPSCharmContexts) -> None:
        ...
        try:
            process = container.exec(
                ["a2dissite", "heat-api-cfn"], timeout=5 * 60
            )
            out, warnings = process.wait_output()
            if warnings:
                for line in warnings.splitlines():
            ...
            logger.exception("Failed to disable heat-api-cfn site in apache")
        super().init_service(context)
