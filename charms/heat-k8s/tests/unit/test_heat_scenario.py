...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → heat.conf is rendered in heat-api."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "heat-api", "/etc/heat/heat.conf"
        )

    def test_cfn_config_file_written(self, ctx, complete_state):
        """All relations present → heat-api-cfn.conf is rendered in heat-api-cfn."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "heat-api-cfn", "/etc/heat/heat-api-cfn.conf"
        )
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        """Pebble-ready on heat-api adds a layer and starts the service."""
        container = complete_state.get_container("heat-api")
        ...
        containers = [
            k8s_container(
                ...
                execs=[
                    ...
                    testing.Exec(
                        command_prefix=["heat-manage"], return_code=0
                    ),
                ],
            ),
            k8s_container(
            ...
            k8s_container("heat-engine"),
        ]
        state_in = testing.State(leader=True, containers=containers)
        container = state_in.get_container("heat-api")
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        containers = [
            k8s_container("heat-api", can_connect=False),
            k8s_container("heat-api-cfn", can_connect=False),
            k8s_container("heat-engine", can_connect=False),
        ]
        state_in = testing.State(leader=True, containers=containers)
