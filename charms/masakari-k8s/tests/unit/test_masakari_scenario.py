...
def _all_containers_disconnected():
    """All three containers with can_connect=False."""
    return [
        k8s_api_container(
            "masakari-api",
            can_connect=False,
            extra_execs=[
                testing.Exec(
                    command_prefix=["masakari-manage"], return_code=0
                ),
            ],
        ),
        k8s_container(
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → masakari.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("masakari-api")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("masakari-api")
        assert "masakari-api" in out_container.layers
        layer = out_container.layers["masakari-api"]
        assert "wsgi-masakari-api" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("wsgi-masakari-api") == (
            testing.pebble.ServiceStatus.ACTIVE
        )

    def test_pebble_ready_without_relations_blocked(self, ctx, containers):
        """Pebble-ready but no relations → blocked."""
        api_container = [c for c in containers if c.name == "masakari-api"][0]
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        state_in = testing.State(
            leader=True, containers=_all_containers_disconnected()
        )
