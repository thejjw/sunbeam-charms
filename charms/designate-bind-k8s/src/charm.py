...
class BindRndcProvidesRelationHandler(sunbeam_rhandlers.RelationHandler):
    ...
    def refresh_address(self):
        ...
        Every unit publishes its own headless service DNS
        address as unit-level `host`.
        """
        unit_host = socket.getfqdn()
...
class BindOperatorCharm(sunbeam_charm.OSBaseOperatorCharmK8S):
    ...
    def __init__(self, *args):
        ...
        service_ports = [
            core_v1.ServicePort(
                53, appProtocol="domain", name="bind-udp", protocol="UDP"
            ),
            core_v1.ServicePort(
                53, appProtocol="domain", name="bind-tcp", protocol="TCP"
            ),
            core_v1.ServicePort(
