...
"""Scenario (ops.testing state-transition) tests for neutron-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → neutron.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "neutron-server", "/etc/neutron/neutron.conf"
        )

    def test_ml2_config_file_written(self, ctx, complete_state):
        """All relations present → ml2_conf.ini is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out,
            ctx,
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        """Pebble-ready adds a layer and starts the neutron-server service."""
        container = complete_state.get_container("neutron-server")
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = testing.Container(name="neutron-server", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
