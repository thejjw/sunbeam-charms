...
class BaremetalConfigContext(sunbeam_ctxts.ConfigContext):
    ...
    def context(self) -> dict:
        ...
        switch_configs = self.charm.baremetal_config.interface.switch_configs

        confs = []
        enabled_devices = []
        for switch_config in switch_configs:
            conf = switch_config.get("conf", "")
            confs.append(conf)

            try:
                config = tomllib.loads(conf)
                enabled_devices.extend(config.keys())
            except tomllib.TOMLDecodeError as ex:
                logger.error("Could not decode TOML. Error: %s", ex)
                raise sunbeam_guard.BlockedExceptionError(
                    f"Invalid content in secret baremetal-switch-config secret. Check logs."
                )
    ...
    def get_additional_files(self) -> Dict[str, str]:
        """Get additional files from the baremetal-switch-config relation.

        :return: (filepath, contents) dictionary of additional files from the relation.
        :rtype: dict
        """
        switch_configs = self.charm.baremetal_config.interface.switch_configs
        ...
        for switch_config in switch_configs:
            try:
                config = tomllib.loads(switch_config.get("conf"))
            except tomllib.TOMLDecodeError as ex:
                logger.error("Could not decode TOML. Error: %s", ex)
                raise sunbeam_guard.BlockedExceptionError(
                    f"Invalid content in baremetal-switch-config secret. Check logs."
                )
            ...
            for name in config.keys():
                ...
                if dict_key not in switch_config:
                    raise sunbeam_guard.BlockedExceptionError(
                        f"Missing '{dict_key}' additional file from baremetal-switch-config secret."
                    )

                additional_files[key_filename] = switch_config[dict_key]
...
class SwitchConfigRequiresHandler(sunbeam_rhandlers.RelationHandler):
    ...
    def setup_event_handler(self):
        ...
        handler = sunbeam_tracing.trace_type(switch_config.SwitchConfigRequires)(
            self.charm,
            self.relation_name,
        )
        self.framework.observe(
...
class NeutronServerPebbleHandler(sunbeam_chandlers.ServicePebbleHandler):
    ...
    def write_config(
        ...
        additional_files = context.baremetal.get_additional_files()
        updated_files = []
