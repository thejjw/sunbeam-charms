...
"""Scenario (ops.testing state-transition) tests for openstack-images-sync-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → config.yaml is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out,
            ctx,
    ...
    def test_http_sync_conf_written(self, ctx, complete_state):
        """All relations present → http-sync.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out,
            ctx,
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("openstack-images-sync")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("openstack-images-sync")
        assert "openstack-images-sync" in out_container.layers
        layer = out_container.layers["openstack-images-sync"]
        assert "images-sync" in layer.to_dict().get("services", {})
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = k8s_container("openstack-images-sync", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
...
class TestWaitingNonLeader:
    """Non-leader behaviour without peer relation."""
    ...
    def test_waiting_non_leader(
        ...
        This charm has no peer relation, so is_leader_ready() always
        returns True.  However configure_app_non_leader checks
        self.bootstrapped() which requires unit_bootstrapped to have
        been set in a *previous* run.  On the very first run,
        unit_bootstrapped is set by configure_unit before
        configure_app runs, so the non-leader should reach active.
        """
        state_in = testing.State(
            leader=False,
        ...
        # Without a peer relation the non-leader is not gated on leader
        # readiness, so it proceeds to active.
        assert state_out.unit_status == testing.ActiveStatus("")
