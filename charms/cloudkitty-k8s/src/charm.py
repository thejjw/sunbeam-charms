...
class LokiLoggingRelationHandler(sunbeam_rhandlers.RelationHandler):
    ...
    def setup_event_handler(self) -> ops.framework.Object:
        ...
        self.loki_consumer = LokiPushApiConsumer(self.charm, relation_name=self.relation_name)
    ...
    def context(self) -> dict:
        ...
        """
        endpoints = self.interface.loki_endpoints if hasattr(self.interface, "loki_endpoints") else []
        loki_url = ""
        ...
        logger.info(f"RELATION HANDLER EXTRACED ENDPOINT: '{loki_url}'")
        return {
            "logging_endpoints": loki_url
        }
...
class CloudkittyWSGIPebbleHandler(sunbeam_chandlers.WSGIPebbleHandler):
    ...
    def wsgi_conf(self) -> str:
        ...
        return (
            f"/etc/apache2/sites-available/wsgi-{self.service_name}.conf"
        )
    ...
    def default_container_configs(self) -> List[sunbeam_core.ContainerConfigFile]:
        return [
