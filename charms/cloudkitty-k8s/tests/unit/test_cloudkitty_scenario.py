...
"""Scenario (ops.testing state-transition) tests for cloudkitty-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → cloudkitty.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        out_container = state_out.get_container("cloudkitty")
        assert "cloudkitty" in out_container.layers
        layer = out_container.layers["cloudkitty"]
        assert "wsgi-cloudkitty-api" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("wsgi-cloudkitty-api") == (
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        container = k8s_api_container("cloudkitty", can_connect=False)
        state_in = testing.State(leader=True, containers=[container])
