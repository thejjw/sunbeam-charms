...
class TestCloudkittyOperatorCharm(test_utils.CharmTestCase):
    ...
    def test_all_relations(self):
        ...
        setup_cmds = [
            ["a2ensite", "wsgi-cloudkitty-api"],
            ["sudo", "-u", "cloudkitty", "cloudkitty-manage", "db", "upgrade"],
        ]
        for cmd in setup_cmds:
            self.assertIn(cmd, self.container_calls.execute["cloudkitty-api"])
        config_files = [
            "/etc/apache2/sites-available/wsgi-cloudkitty-api.conf",
            "/etc/cloudkitty/cloudkitty.conf",
        ]
        for f in config_files:
            self.check_file("cloudkitty-api", f)
