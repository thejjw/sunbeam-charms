...
class TestEpaOrchestratorCharm(test_utils.CharmTestCase):
    ...
    def test_snap_name_property(self):
        ...
        self.harness.begin()
        self.harness.update_config({"snap-name": "test-snap"})

        self.assertEqual(self.harness.charm.snap_name, "test-snap")

    def test_snap_name_property_default(self):
        """Test snap_name property returns default value."""
        self.harness.begin()

        # Default from charmcraft.yaml is "hello-world"
        self.assertEqual(self.harness.charm.snap_name, "hello-world")
