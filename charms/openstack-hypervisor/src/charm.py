    ...
    def _connect_to_epa_orchestrator(self):
        ...
        try:
            ...
            if not epa_orchestrator.present:
                logger.info("epa-orchestrator not installed, skipping connection")
                return
        except snap.SnapNotFoundError:
            logger.info("epa-orchestrator not found in snap cache, skipping connection")
            return

        try:
            hypervisor.connect("epa-info", slot="epa-orchestrator:epa-info")
            logger.info("Successfully connected openstack-hypervisor to epa-orchestrator")
        except snap.SnapError as e:
            logger.warning(f"Failed to connect to epa-orchestrator: {e.message}")
