...
class HypervisorOperatorCharm(sunbeam_charm.OSBaseOperatorCharm):
    ...
    def _on_refresh_snap_action(self, event: ActionEvent) -> None:
        ...
        try:
            ...
            if hypervisor.present and hypervisor.channel:
                channel = sunbeam_charm.resolve_snap_channel(
                    hypervisor.channel, channel
                )
            hypervisor.unhold()
            hypervisor.ensure(
    ...
    def _disable_system_ovs(self) -> bool:
        ...
        OVS crashes if there are multiple conflicting installations, so mask
        system OVS services before using the MicroOVN snap installation.
