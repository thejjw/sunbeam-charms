...
def is_schedule_valid(schedule: str) -> bool:
    """Return a boolean to check if the cron schedule is valid."""
    return not get_schedule_error(schedule)


def get_schedule_error(schedule: str) -> Optional[str]:
    """If there is an error with the schedule, return a message."""
    ...
    if len(schedule.split()) == 6:
        return "This cron does not support seconds in schedule (6 fields)."
    ...
    if t2 - t1 <= 15 * 60:  # 15 minutes in seconds
        return "Schedule repeats faster than 15 minutes."
