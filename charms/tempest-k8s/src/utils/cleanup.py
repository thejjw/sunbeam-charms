...
def _get_test_projects_in_domain(conn, domain_id) -> list[str]:
...
def _cleanup_compute_resources(conn, project_id) -> None:
...
def _cleanup_block_resources(conn, project_id) -> None:
...
def _cleanup_images(conn, project_id) -> None:
...
def _cleanup_networks_resources(conn, project_id) -> None:
...
def _cleanup_stacks(conn, project_id) -> None:
...
def run_quick_cleanup(env) -> None:
    ...
    for project_id in projects:
        for func in cleanup_funcs:
            try:
                ...
                failure_message += f"\nError calling {func.__name__}: {e}"

    if failure_message:
        raise CleanUpError(failure_message)


def run_extensive_cleanup(env) -> None:
