...
class TempestOperatorCharm(sunbeam_charm.OSBaseOperatorCharmK8S):
    ...
    def get_schedule(self) -> str:
        ...
        then return an empty (disabled) schedule string.
        """
        schedule = self.config["schedule"]
        if not is_schedule_valid(schedule):
            schedule = ""

        # TODO: check if observability relations are ready
        return schedule

    def build_tempest_context(self) -> TempestConfigurationContext:
        """Get the context for rendering template files for this charm."""
        context = TempestConfigurationContext(self, "tempest")
        return context

    @property
    def config_contexts(self) -> List[ConfigContext]:
        """Generate list of configuration adapters for the charm."""
        cadapters = super().config_contexts
        cadapters.extend([self.build_tempest_context()])
        return cadapters
