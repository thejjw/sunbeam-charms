...
```
import charms.manila_cephfs_k8s.v0.ceph_nfs as ceph_nfs
...
class CephNfsClientCharm(CharmBase):
    def __init__(self, *args):
        ...
        self.framework.observe(
            self._ceph_nfs.on.goneaway,
            self._on_ceph_nfs_goneaway,
        )
    ...
    def _on_ceph_nfs_goneaway(self, event):
        '''React to the CephNfsGoneAwayEvent event.

        This event happens when ceph-nfs relation is removed.
        '''
        # ceph-nfs relation has goneaway. Shutdown services if needed.
        pass
```
"""
...
class CephNfsRequires(Object):
    ...
    def get_remote_app_data(self, key: str) -> str | None:
        ...
        if self._ceph_nfs_rel:
            data = self._ceph_nfs_rel.data[self._ceph_nfs_rel.app]
            return data.get(key)
