...
"""Scenario (ops.testing state-transition) tests for manila-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → manila.conf is rendered in both containers."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
            state_out, ctx, "manila-api", "/etc/manila/manila.conf"
        )
        assert_config_file_exists(
            state_out, ctx, "manila-scheduler", "/etc/manila/manila.conf"
        )
...
class TestPebbleReady:
    ...
    def test_pebble_ready_api(self, ctx, complete_state):
        """Pebble-ready on manila-api configures the WSGI service."""
        container = complete_state.get_container("manila-api")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("manila-api")
        assert "manila-api" in out_container.layers
        layer = out_container.layers["manila-api"]
        assert "wsgi-manila-api" in layer.to_dict().get("services", {})

    def test_pebble_ready_scheduler(self, ctx, complete_state):
        """Pebble-ready on manila-scheduler configures the scheduler service."""
        container = complete_state.get_container("manila-scheduler")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("manila-scheduler")
        assert "manila-scheduler" in out_container.layers
        layer = out_container.layers["manila-scheduler"]
        assert "manila-scheduler" in layer.to_dict().get("services", {})

    def test_pebble_ready_without_relations_blocked(self, ctx):
        """Pebble-ready but no relations → blocked."""
        api = k8s_api_container("manila-api")
        scheduler = k8s_container("manila-scheduler", execs=[sudo_exec()])
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        api = k8s_api_container("manila-api", can_connect=False)
        scheduler = k8s_container(
            "manila-scheduler", can_connect=False, execs=[sudo_exec()]
        )
        state_in = testing.State(leader=True, containers=[api, scheduler])
