...
class ManilaProvides(Object):
    ...
    def __init__(self, charm: CharmBase, relation_name: str, share_protocol: str):
    ...
    def _on_manila_relation_joined(self, event: RelationJoinedEvent):
        ...
        relation.data[self.model.unit][SHARE_PROTOCOL] = self.share_protocol
        self.on.manila_connected.emit()
...
class ManilaRequires(Object):
    ...
    def get_remote_app_data(self, key: str) -> str | None:
        ...
        if self._manila_rel:
            data = self._manila_rel.data[self._manila_rel.app]
            return data.get(key)
