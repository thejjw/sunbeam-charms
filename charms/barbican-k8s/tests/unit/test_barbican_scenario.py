...
"""Scenario (ops.testing state-transition) tests for barbican-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → barbican.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
...
class TestPebbleReady:
    ...
    def test_pebble_ready_api(self, ctx, complete_state):
        """Pebble-ready on barbican-api configures the WSGI service."""
        container = complete_state.get_container("barbican-api")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)
        assert state_out.unit_status == testing.ActiveStatus("")

    def test_pebble_ready_worker(self, ctx, complete_state):
        """Pebble-ready on barbican-worker configures the worker service."""
        container = complete_state.get_container("barbican-worker")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)
        assert state_out.unit_status == testing.ActiveStatus("")
    ...
    def test_pebble_ready_without_relations_blocked(self, ctx):
        ...
        worker = k8s_container("barbican-worker")
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        worker = k8s_container("barbican-worker", can_connect=False)
        state_in = testing.State(
            leader=True,
            containers=[api, worker],
            secrets=[vault_kv_nonce_secret()],
