...
"""Scenario (ops.testing state-transition) tests for gnocchi-k8s."""
...
class TestAllRelations:
    ...
    def test_config_file_written(self, ctx, complete_state):
        """All relations present → gnocchi.conf is rendered."""
        state_out = ctx.run(ctx.on.config_changed(), complete_state)
        assert_config_file_exists(
...
class TestPebbleReady:
    ...
    def test_pebble_ready_configures_container(self, ctx, complete_state):
        ...
        container = complete_state.get_container("gnocchi-api")
        state_out = ctx.run(ctx.on.pebble_ready(container), complete_state)

        assert state_out.unit_status == testing.ActiveStatus("")

        out_container = state_out.get_container("gnocchi-api")
        assert "gnocchi-api" in out_container.layers
        layer = out_container.layers["gnocchi-api"]
        assert "wsgi-gnocchi-api" in layer.to_dict().get("services", {})

        assert out_container.service_statuses.get("wsgi-gnocchi-api") == (
    ...
    def test_pebble_ready_without_relations_blocked(self, ctx):
        ...
        api_container = k8s_api_container("gnocchi-api")
        metricd_container = k8s_container(
            "gnocchi-metricd", execs=[sudo_exec()]
        )
        state_in = testing.State(
            leader=True, containers=[api_container, metricd_container]
        )
...
class TestBlockedWhenNoRelations:
    ...
    def test_blocked_when_no_relations(self, ctx):
        ...
        api_container = k8s_api_container("gnocchi-api", can_connect=False)
        metricd_container = k8s_container(
            "gnocchi-metricd", can_connect=False, execs=[sudo_exec()]
        )
        state_in = testing.State(
            leader=True, containers=[api_container, metricd_container]
        )
