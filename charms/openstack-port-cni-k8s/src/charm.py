...
class OpenstackPortCniCharm(sunbeam_charm.OSBaseOperatorCharm):
    ...
    def get_relation_handlers(
        ...
        if self.can_add_handler("identity-credentials", handlers):
            self.id_creds = (
                sunbeam_rhandlers.IdentityCredentialsRequiresHandler(
                    self,
                    "identity-credentials",
                    self.configure_charm,
                    mandatory="identity-credentials"
                    in self.mandatory_relations,
                )
            )
            handlers.append(self.id_creds)
    ...
    def _sync_ca_bundle(self) -> None:
        ...
        try:
            client.apply(secret, field_manager="openstack-port-cni", force=True)
            log.info("CA bundle Secret updated")
        except Exception as exc:
            log.error("Failed to update CA bundle Secret: %s", exc)
    ...
    def _sync_openstack_credentials(self) -> None:
        ...
        """
        try:
            auth_url = self.id_creds.interface.internal_endpoint
            username = self.id_creds.interface.username
            password = self.id_creds.interface.password
            project_name = self.id_creds.interface.project_name
            project_id = self.id_creds.interface.project_id
            user_domain_name = (
                self.id_creds.interface.user_domain_name or "service_domain"
            )
            project_domain_name = (
                self.id_creds.interface.project_domain_name or "service_domain"
            )
        except (AttributeError, KeyError, TypeError):
            log.warning("Keystone credentials not yet fully available")
            return

        if not all([auth_url, username, password, project_name]):
            log.warning(
                "Keystone credentials incomplete, skipping Secret sync"
            )
            return
        ...
        try:
            ...
            log.error("Failed to update OpenStack credentials Secret: %s", exc)
            return
    ...
    def _on_update_status(self, event: ops.UpdateStatusEvent) -> None:
        """Re-check DaemonSet readiness and update unit status.

        Runs every update-status interval (default 5 min) so the charm
        transitions to active without needing a full configure_charm run or
        an external juju status poll.
        """
        waiting = self._daemonset_waiting()
        if waiting:
            self.unit.status = ops.WaitingStatus(waiting)
        else:
            # Only promote to active if we were waiting on the DS;
            # don't stomp over an error set by a previous hook failure.
            if isinstance(self.unit.status, ops.WaitingStatus):
