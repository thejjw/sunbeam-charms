...
def connect_snap_interfaces() -> None:
    ...
    abort the charm start-up. If the *snap* binary is absent (e.g. during
    unit tests in CI), the function exits quietly.
    """
    if shutil.which("snap") is None:
        logger.debug("'snap' command not found – skipping interface connects")
        return

    interfaces = [
        "network",
        "network-bind" "network-observe",
        "mount-observe",
        "block-devices",
        "system-observe",
        "hardware-observe",
